...
%API Description
{
       The \FUNC{shmalloc} and \FUNC{shmem\_malloc}  routines return a pointer to a block of at least size
       ...
       The  \FUNC{shmemalign} and \FUNC{shmem\_align} routines allocate a block in the symmetric heap that
       has a byte alignment specified by the alignment argument.

       The \FUNC{shfree} and \FUNC{shmem\_free} routines cause the block to which \VAR{ptr} points to be
       deallocated, that is, made available for further allocation.  If ptr is
       a null pointer, no action occurs. 
              
       The  \FUNC{shrealloc} and  \FUNC{shmem\_realloc} routines change the size of the block to which ptr
       ...
       If \VAR{ptr} is a \CONST{NULL} pointer, the \FUNC{shrealloc}/\FUNC{shmem\_realloc} routine behaves like the \FUNC{shmalloc}/\FUNC{shmem\_malloc} routine for the specified size.  If size  is \CONST{0} and ptr is not a \CONST{NULL} pointer, the block to which it points is freed. If the space cannot be allocated, the block to which ptr points is unchanged.

       The \FUNC{shmalloc}, \FUNC{shfree}, and \FUNC{shrealloc} routines are provided  so that multiple \ac{PE}s in a program can allocate symmetric, remotely
...
%API Description Table
{
		%Return Value
		\desR{
					 The \FUNC{shmalloc} routine returns a pointer to the allocated space (which should be identical on all \ac{PE}s); otherwise, it returns a \CONST{NULL} pointer.

					 The \FUNC{shfree} routine returns no value.

					 The \FUNC{shrealloc} routine returns a pointer to the allocated space (which may have moved); otherwise, it returns a null pointer.
		}
		%NOTES
