...
%API Description
{
       ...
       block is indeterminate.  If \VAR{ptr} is a \CONST{NULL} pointer, the \FUNC{shrealloc} function behaves like the \FUNC{shmalloc} function for the specified size.  If size  is \CONST{0} and ptr is not a \CONST{NULL} pointer, the block to which it points is freed.  Otherwise, if ptr does not match a pointer earlier  returned
...
%API Description Table
{
		%Return Value
		\desR{
					 The \FUNC{shmalloc} function returns a pointer to the allocated space (which should be identical on all \ac{PE}s); otherwise, it returns a \CONST{NULL} pointer (with \VAR{malloc\_error} set).

					 The \FUNC{shfree} function returns no value.

					 The \FUNC{shrealloc} function returns a pointer to the allocated space (which may have moved); otherwise, it returns a null pointer (with
