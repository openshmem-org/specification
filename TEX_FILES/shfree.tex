...
%API Description
{
       The and \FUNC{shmem\_malloc}  routines return a pointer to a block of at least size
       ...
       The and \FUNC{shmem\_align} routines allocate a block in the symmetric heap that
       has a byte alignment specified by the alignment argument.

       The and \FUNC{shmem\_free} routines cause the block to which \VAR{ptr} points to be
       deallocated, that is, made available for further allocation.  If ptr is
       a null pointer, no action occurs. 
              
       The  and \FUNC{shmem\_realloc} routines change the size of the block to which ptr
       ...
       If \VAR{ptr} is a \CONST{NULL} pointer, the \FUNC{shmem\_realloc} routine behaves like the \FUNC{shmem\_malloc} routine for the specified size.  If size  is \CONST{0} and ptr is not a \CONST{NULL} pointer, the block to which it points is freed. If the space cannot be allocated, the block to which ptr points is unchanged.
...
%API Description Table
{
		%Return Value
		\desR{
					 The \FUNC{shmem\_malloc} routine returns a pointer to the allocated space (which should be identical on all \ac{PE}s); otherwise, it returns a \CONST{NULL} pointer.
