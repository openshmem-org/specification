...
\desB{
	...
	\argRow{IN}{value}{The value to be atomically added to \dest. If you are using \CorCpp, the type of \VAR{value} should match that  implied  in  the SYNOPSIS  section.  If you are using \Fortran, it must be of type integer with an element size of \dest.}
...
%API Description
{The \FUNC{shmem\_add} routine performs an atomic add operation. It adds \VAR{value}
 to \dest{} on \ac{PE} \VAR{pe} and atomically updates the \dest{} without returning the value.
 } 
{
 \hfill \\
