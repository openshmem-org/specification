...
%API description
 { 
 The \FUNC{shmem\_quiet} routine ensures completion of \PUT{}, \acp{AMO}, and store operations on symmetric data issued by the calling \ac{PE}. All \PUT{}, \acp{AMO}, store operations to symmetric data objects are guaranteed to be completed and visible to all \ac{PE}s when \FUNC{shmem\_quiet} returns. 
...
\notesB{ 
       ...
       several \PUT{}, \acp{AMO}, and store operations to symmetric data objects initiated by the calling \ac{PE}. For example,  you might use \FUNC{shmem\_quiet} to await delivery of 
       a block of data before issuing  another \PUT{}, which sets a completion flag on another \ac{PE}.
...
%For each example, you can enter it as an item.
	...
	{The  following  simple  example uses  \FUNC{shmem\_quiet}  in a \CorCpp{} program: }	 
       ...
       {\VAR{Put1} will be completed and visible before \VAR{put3} and \VAR{put2} will be completed and visible before \VAR{put4}.}
}
\eAPI
