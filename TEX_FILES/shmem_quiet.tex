...
%API description
 { 
 The \FUNC{shmem\_quiet} routine ensures completion of \PUT{}, \acp{AMO}, and store operations on symmetric data issued by the calling \ac{PE}. All \PUT{}, \acp{AMO}, \cbstart memory \cbend store operations to symmetric data objects are guaranteed to be completed and visible to all \ac{PE}s when \FUNC{shmem\_quiet} returns. 
