\bAPI{SHMEM\_QUIET}{Waits  for  completion of all outstanding remote writes issued by a \ac{PE}.}
...
%API description
 { 
 The \FUNC{shmem\_quiet} routine ensures ordering of put (remote write) operations.  All put operations  issued  to any \ac{PE} prior to the call to \FUNC{shmem\_quiet} are guaranteed to be visible to all other \ac{PE}s no later than any   subsequent memory load or  store,  remote  put  or  get,  or synchronization operations that follow the call to \FUNC{shmem\_quiet}.
}
...
\notesB{ 
       \FUNC{shmem\_quiet} is most useful as a way of ensuring ordering of delivery of
       several put operations. For example,  you might use \FUNC{shmem\_quiet} to
