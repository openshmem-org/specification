...
\hfill \\
 \desTBC{ When calling from \Fortran, the \target{} date types are as follows:}
		  ...
		  \cRow{shmem\_comp4\_sum\_to\_all}{Complex,\cbstart with an element size equal to two 4-byte real values.}
		  ...
		  \cRow{shmem\_comp8\_prod\_to\_all}{ Complex, with an element size equal to two 8-byte real values.\cbend}
		  ...
		  \cRow{shmem\_real16\_prod\_to\_all}{Real, \cbstart with an element size of 16 bytes.}
		  \cRow{shmem\_int8\_or\_to\_all}{Integer, with an element size of 8 bytes.}
		  \cRow{shmem\_int4\_or\_to\_all}{Integer, with an element size of 4 bytes.\cbend}
...
\notesB{  
 ...
 the \activeset{} while any of the \ac{PE}s participate in processing of \cbstart an \cbend
 ...
 assured only by doing some type of synchronization\cbstart .\cbend 
% However, in the
