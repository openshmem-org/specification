...
%API Description
{
    ...
    \acp{PE} have entered \FUNC{shmem\_finalize}. \FUNC{shmem\_finalize} must
    be the last \openshmem library call encountered in the \openshmem portion
    of a program. A call to \FUNC{shmem\_finalize} will release any resources
    initialized by the most recent call to \FUNC{shmem\_init} that has not
    already been matched with a call to \FUNC{shmem\_finalize}. All processes
    and threads that represent the \acp{PE} will still exist after the call to
...
%NOTES
\notesB{
    \FUNC{shmem\_finalize} releases resources used by the \openshmem library
