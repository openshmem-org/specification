...
{   
    ...
    \openshmem program which it initialized, each call to \FUNC{shmem\_init}
    must be matched with a call to \FUNC{shmem\_finalize}. If there are
    multiple calls to \FUNC{shmem\_init} in an \openshmem program without
    intervening calls to \FUNC{shmem\_finalize} between them, the additional
    \FUNC{shmem\_init} calls have no effect.
}
...
\notesB{ 
    ...
    encouraged to use \FUNC{shmem\_init}. Direct replacement of
    \FUNC{start\_pes} with \FUNC{shmem\_init} in \openshmem programs with no
    further changes is possible; there is an implicit \FUNC{shmem\_finalize} at
    the end of main.
}
}%end of DesB
%Example
