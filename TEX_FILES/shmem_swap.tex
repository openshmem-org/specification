...
\argRow{IN}{value}{Value to be atomically written to the remote  \ac{PE}.   value  is
...
%API description
{          shmem\_swap  performs  an atomic swap operation. It writes value value
...
\cRow{SHMEM\_INT4\_SWAP}{4-byte integer}
\cRow{SHMEM\_INT8\_SWAP}{ 8-byte integer}
\cRow{SHMEM\_REAL4\_SWAP}{4-byte real}
\cRow{SHMEM\_REAL8\_SWAP}{8-byte real}
}
 %Return Values     
\desR{
...
\exampleB{
    ...
    {The following call ensures that the first \ac{PE} to execute the
     conditional swap will successfully write its \ac{PE} number to race\_winner
     on \ac{PE} 0.}
