...
\notesB{The \FUNC{shmem\_ptr} routine is available only on systems where ordinary
       memory loads and stores are used to implement \openshmem put and get
       routines. When calling \FUNC{shmem\_ptr}, you pass the address on the calling \ac{PE}  for a symmetric array on the remote \ac{PE}.}
}
