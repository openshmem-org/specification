...
{
 \desR{
        \color{red}\sout{returns a pointer to the data object on the specified  remote \ac{PE}.
        If \target{} is not remotely accessible, a \CONST{NULL} pointer is
        returned.}
        \color{blue}The return value is a non-NULL address of the \target{} data object when it is 
        accessible using memory loads and stores in addition to \openshmem 
        \color{red}\sout{put and get}\color{blue}~ operations.
        Otherwise, a NULL address is returned.\color{black}
}
\notesB{
        \color{red}\sout{The \FUNC{shmem\_ptr} function is available only on systems where
        ordinary memory loads and stores are used to implement \openshmem
        put and get operations.}\color{black}
        When calling \FUNC{shmem\_ptr}, \color{red}\sout{you pass}
        \color{blue}\target{} is \color{black} the address 
        \color{red}\sout{on the calling \ac{PE} for a symmetric array on the remote \ac{PE}.}
        \color{blue}of the referenced symmetric data object on the calling \ac{PE}.\color{black}
        }
}
