...
void shmem_fcollect64(void *dest, const void *source,	size_t nelems, int PE_start, int logPE_stride, int PE_size, long *pSync); %*\synCE    %DO NOT DELETE. THIS LINE IS NOT A COMMENT
...
CALL SHMEM_FCOLLECT64(dest, source, nelems, PE_start, logPE_stride, PE_size, pSync) %*\synFE   %DO NOT DELETE. THIS LINE IS NOT A COMMENT  

% Arguments table. If no arguments you can use \argRow{None}{}{} 
\desB{  
\argRow{OUT}{dest}{A symmetric array.	 The \dest{} argument must be large
...
\argRow{IN}{nelems}{The number of elements in the \source{} array. \VAR{nelems} must be
...
\argRow{IN}{logPE\_stride}{The log (base \CONST{2}) of the stride between consecutive \ac{PE} numbers in the \activeset. \VAR{logPE\_stride} must be of
...
\argRow{IN}{pSync}{A symmetric  work array.  In \CorCpp, \VAR{pSync} must be of type
	    ...
	    \CONST{SHMEM\_SYNC\_VALUE} in \Fortran{} before any of the \ac{PE}s in the \activeset{} enter \FUNC{shmem\_collect} or \FUNC{shmem\_fcollect}.}
}
%API description
{   
\OSH{} \FUNC{collect} and \FUNC{fcollect} routines concatenate
\VAR{nelems} \CONST{64}-bit or \CONST{32}-bit data items from the \source{} array into the \dest{} array, over the set of \ac{PE}s defined by \VAR{PE\_start}, \VAR{log2PE\_stride}, and \VAR{PE\_size}, in processor number order. The resultant \dest{} array
contains the contribution from \ac{PE} \VAR{PE\_start} first, then the contribution
from \ac{PE} \VAR{PE\_start} + \VAR{PE\_stride} second, and so on. The collected result is written to the \dest{} array for all \ac{PE}s in the \activeset.

The \FUNC{fcollect} routines require that \VAR{nelems} be the same value in all
participating \ac{PE}s, while the \FUNC{collect} routines allow \VAR{nelems} to vary from
\ac{PE} to \ac{PE}.
...
The values of arguments \VAR{PE\_start}, \VAR{logPE\_stride}, and \VAR{PE\_size} must be equal on all \ac{PE}s in the \activeset. The same \dest{} and \source{} arrays
...
% 		    Fortran, it must be a default integer value.
% 
%        IN	logPE_stride The log (base 2) of the stride between consecutive 
