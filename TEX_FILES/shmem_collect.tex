\bAPI{SHMEM\_COLLECT}{Concatenates blocks of data from multiple processing elements (PEs) to an array in every PE.}
...
\argRow{OUT}{target}{A symmetric array.	The  target  argument  must  be	 large
	    ...
	    For shmem\_collect8, shmem\_collect64, shmem\_fcollect8,  and
	    shmem\_fcollect64, any data type with an element size of 64
	    bits.  Fortran derived types, Fortran character type,  and
	    C/C++  structures  are not permitted.  For shmem\_collect4,
	    shmem\_collect32,  shmem\_fcollect4,	and  shmem\_fcollect32,
...
\argRow{IN}{logPE\_stride}{The log (base 2) of the stride between consecutive virtual
	    PE	numbers	 in  the  active set.  logPE\_stride must be of
...
\argRow{IN}{pSync}{A symmetric  work array.  In C/C++, pSync must be of type
	    long and size \_SHMEM\_COLLECT\_SYNC\_SIZE.  In Fortran, pSync
	    ...
	    with   the	 value	 \_SHMEM\_SYNC\_VALUE   in	   C/C++    or
	    SHMEM\_SYNC\_VALUE  in  Fortran before any of the PEs in the
	    active set enter shmem\_barrier().}
}
%API description
{   
...
%Example
\exampleB{
    \exampleITEM{The following shmem\_collect example is for C/C++ programs:}
    {./EXAMPLES/shmem_collect_example.c}
    {}
    %For each example, you can enter it as an item.
    \exampleITEM{The following SHMEM\_COLLECT example is for Fortran programs:}
