...
int main(void)
{
   ...
   shmem_team_t         twos_team   = SHMEMX_TEAM_INVALID;
   shmem_team_t         threes_team = SHMEMX_TEAM_INVALID;
   ...
   if (npes > 2)
       shmem_team_split_strided(SHMEMX_TEAM_WORLD, 2, 2, (npes-1) / 2, config,
                                 0, &twos_team);

   if (npes > 3)
       shmem_team_split_strided(SHMEMX_TEAM_WORLD, 3, 3, (npes-1) / 3, config,
                                 0, &threes_team);
