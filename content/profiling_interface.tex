...
it is also required that the \FUNC{pshmem\_} version be supplied 
...
\FUNC{pshmem\_} interfaces because of the limitation from \Cstd 
\FUNC{\_Generic} semantics. 
...
profiling functions to be called from within a \openshmem function 
...
operation may lead to count a profiling function for 
...
cannot have any name-shifted \FUNC{pshmem\_} routines in 
the profiling library. Furthermore, because of the limitations in 
\Cstd \FUNC{\_Generic} semantics, the \Cstd[11] fixed-width type 
bindings may map to \Cstd basic types (e.g. \VAR{int\_32t} may map 
to \VAR{int}), which also limits the profiling library to 
