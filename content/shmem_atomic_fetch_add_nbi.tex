\color{Green}
\apisummary{
    This nonblocking routine performs an atomic fetch-and-add operation on a
    remote data object.
}
...
\apiargument{IN}{value}{The value to be atomically added to \VAR{dest}.
    \VAR{value} is the same type as \dest.}
...
\apidescription{
    ...
    of the fetch and the update. This routine returns after posting the
    operation. The operation is considered complete after a subsequent call to
    \FUNC{shmem\_quiet}. At the completion of \FUNC{shmem\_quiet}, these
    routines add \VAR{value} to \VAR{dest} on \VAR{pe} and fetch the previous
    contents of \VAR{dest} as an atomic operation into the \VAR{fetch} local
    data object.
}
