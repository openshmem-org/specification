...
\begin{C11synopsis}
size_t @\FuncDecl{shmem\_wait\_until\_some}@(TYPE * restrict ivars, size_t nelems,
    size_t * restrict indices, int * restrict status, int cmp, TYPE cmp_value);
\end{C11synopsis}
where \TYPE{} is one of the point-to-point synchronization types specified by
Table \ref{p2psynctypes}.

\begin{Csynopsis}
size_t @\FuncDecl{shmem\_\FuncParam{TYPENAME}\_wait\_until\_some}@(TYPE * restrict ivars, size_t nelems,
    size_t * restrict indices, int * restrict status, int cmp, TYPE cmp_value);
\end{Csynopsis}
...
\apidescription{ 
    ...
    wait condition at the calling \ac{PE}.  This routine tests all
    \VAR{nelem} elements of \VAR{ivars} at least once, and the order in which
    the elements are waited upon is unspecified.
    ...
    satisfied elements.  If the return value is $N$, then the first $N$
    elements of the \VAR{indices} array contain those unique indices that
    satisfied the wait condition.  The array pointed to by \VAR{indices} must
    be at least \VAR{nelems} long.
    ...
    and all elements in \VAR{ivars} are included in the wait set.
}
...
\begin{apiexamples}
  \apicexample
      ...
      \FUNC{shmem\_wait\_until\_all} example above, but may reduce the number of
