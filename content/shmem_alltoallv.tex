...
\apiargument{IN}{t\_offsets}{A symmetric data object with \VAR{PE\_size}
    ...
    \VAR{dest}) at which to place the incoming data from \ac{PE} i.}
\apiargument{OUT}{t\_nelems}{A symmetric data object with \VAR{PE\_size} entries.
    On function return, entry i will contain the number of elements received
    from \ac{PE} i.}
...
\apiargument{IN}{s\_nelems}{A symmetric data object with \VAR{PE\_size} entries.
    Entry i specifies the number of elements that will be sent to \ac{PE}
    i.}
...
\apidescription{
    The \FUNC{shmem\_alltoallv} routines are collective routines; each \ac{PE}
    in the \activeset\ exchanges distinct data with all other \acp{PE} in the
    set.  Each \ac{PE} specifies an array of source offsets and source sizes (in
    number of elements) that describe what data to send to each of the other
    \acp{PE} in the set.  The caller also specifies a \VAR{dest} buffer and a
    dest offset data object that specifies where data received from each of the
    other \acp{PE} is deposited. The \VAR{t\_nelems} data object is an output
    parameter. On return from the collective, the \VAR{t\_nelems} data object
    contains the actual amount of data in number of elements the local \ac{PE}
    received from each \ac{PE}.

    The \FUNC{shmem\_alltoallv} routines require each PE to allocate its
    ...
    Before any \ac{PE} calls a \FUNC{shmem\_alltoallv} routine, the following
    ...
    Upon return from a \FUNC{shmem\_alltoallv} routine, the following is true
    for the local PE: The \VAR{dest} and \VAR{t\_nelems} symmetric data objects
    are updated.
}
...
\apicexample
    {This example shows \FUNC{shmem\_alltoallv64} exchanging variable sized
