...
been completed. If the operation is completed, \FUNC{shmem\_req\_wait}
returns zero, and the request object is deallocated and set to
SHMEM\_REQ\_INVALID. If the operation has not been completed,
\FUNC{shmem\_req\_wait} blocks until the operation completes and then returns
zero. If the request object is not valid (i.e., it is set to
SHMEM\_REQ\_INVALID), no operation is performed and a negative value is returned.
