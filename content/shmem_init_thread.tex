...
The allowed values for \VAR{provided} and \VAR{requested} are 
...
\FUNC{shmem\_init\_thread} routine may not be called multiple times in an \openshmem program.
...
\apinotes{
    The \openshmem library can be initialized either by \FUNC{shmem\_init} or
    \FUNC{shmem\_init\_thread}. If the \openshmem library  is initialized by
    \FUNC{shmem\_init}, the library implementation can choose to support one of the 
