...
\begin{apiarguments}
    ...
    \apiargument{IN/OUT}{req}{SHMEM\_REQUEST\_UNINITIALIZED, or a valid request object}
\end{apiarguments}

\apidescription{
    The explicit non-blocking put routines provide a method for copying a
    ...
    parameter is initialized to SHMEM\_REQUEST\_UNINITIALIZED, a new
    request will be created.
    There are no guarantees to the order of operations if multiple successive operations are posted. 
    The operation is considered complete after a subsequent call
    to \FUNC{shmem\_wait\_nbe} for a given request handle returns, or a
    call to \FUNC{shmem\_test\_nbe} for a given request handle returns
    with a non-zero flag.
 }
...
\apidesctable{
    ...
    \apitablerow{shmem\_putmem\_nbe}{Any  data  type.  nelems is scaled in bytes.}
    \apitablerow{shmem\_put8\_nbe}{Any noncharacter type that has a storage size equal to \CONST{8} bits.}
    \apitablerow{shmem\_put32\_nbe}{Any noncharacter type that has a storage size equal to \CONST{32} bits.}
    \apitablerow{shmem\_put64\_nbe}{Any noncharacter type that has a storage size equal to \CONST{64} bits.}
    \apitablerow{shmem\_put128\_nbe}{Any noncharacter type that has a storage size equal to \CONST{128} bits.}
}
\apireturnvalues{
    None.
}
