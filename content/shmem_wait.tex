...
\apinotes{
    ...
    memory model. A memory barrier or load-acquire instruction may be required
    between loads at the target \ac{PE} in order to prevent reordering.  For
    example, such reordering could happen when the target \ac{PE} polls for
    update to a variable, e.g. 'flag,' then accesses another variable, e.g.
    'data,' and expects to see the most recent update to 'data.' For writing
    portable code in such scenarios, \FUNC{shmem\_wait} or
...
\apiimpnotes{
    ...
    load-acquire instructions to guarantee the correct visibility ordering
    between updates to \VAR{ivar} and updates to the rest of the memory.}
}
