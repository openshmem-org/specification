...
\apiargument{IN}{max\_offset}{The maximum amount of data in number of elements
that any \ac{PE} in the \activeset\ will send during the exchange.}
\apiargument{OUT}{t\_nelems}{A symmetric data object with \VAR{PE\_size} entries.
    On function return, entry i will contain the number of elements received from
    \ac{PE} i.}
...
\apiargument{IN}{s\_offsets}{A symmetric data object with \VAR{PE\_size}
    entries. Entry i specifies the number of elements offset (relative to
    \VAR{source}) of the starting address of the data to be sent to \ac{PE} i.}
\apiargument{IN}{s\_nelems}{A symmetric data object with \VAR{PE\_size} entries.
    Entry i specifies the amount of data (in number of elements) that will be
    sent to \ac{PE} i.}
...
\apidescription{
    The \FUNC{shmem\_alltoallv\_max} routines are collective routines; each
    ...
    sizes (in number of elements) that describe what data to send to each of the
    other \acp{PE} in the set.  The caller also specifies a \VAR{dest} buffer
    along with a \VAR{max\_offset} in number elements that indicates the maximum
    amount of data that will be sent by any \ac{PE} during the exchange.  Each
    sending \ac{PE} will then deposit its data into its own \VAR{max\_offset}
    sized region of the \VAR{dest} buffer on the local \ac{PE}.  The
    \VAR{t\_nelems} data object is an output parameter. On return from the
    collective, the \VAR{t\_nelems} data object contains the actual amount of
    data in number of elements the local \ac{PE} received from each \ac{PE}.

    The \FUNC{shmem\_alltoallv\_max} routines require each \ac{PE} to allocate
    ...
    Before any \ac{PE} calls to a \FUNC{shmem\_alltoallv\_max} routine, the
    following conditions must exist (synchronization via a barrier or some other
    method is often needed to ensure this): The \VAR{pSync} array on all
    \acp{PE} in the \activeset{} is not still in use from a prior call to a
    ...
    Upon return from a \FUNC{shmem\_alltoallv\_max} routine, the following is
    true for the local PE: The \VAR{dest} and \VAR{t\_nelems} symmetric data
    objects are updated.
}
...
\apicexample
    {This example shows \FUNC{shmem\_alltoallv\_max64} exchanging variable sized
