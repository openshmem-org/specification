...
\item New API \FUNC{shmem\_sync\_all} and \FUNC{shmem\_sync} to provide PE
...
on the remote PE.\tabularnewline
\hline
Compare and Swap & A generic term for OpenSHMEM fetching AMO routines in which 
the PE initiating the swap atomically gets the old value of the symmetric data object based 
on a value to be compared and copies a new value to the symmetric data object on the 
remote PE.\tabularnewline
\hline
Fetch and Increment & A generic term for OpenSHMEM fetching AMO routines in which 
the PE initiating the increment atomically adds 1 to the symmetric data object on the 
remote PE and returns with the old value.\tabularnewline
\hline
Fetch and Add & A generic term for OpenSHMEM fetching AMO routines in which the 
PE initiating the add specifies the value to be atomically added to the symmetric data 
object on the remote PE and returns with the old value.\tabularnewline
\hline
Fence & A generic term for OpenSHMEM routines a PE can invoke to ensure ordering 
of Puts and AMO to symmetric data objects with respect to a specific destination PE.  
Put and AMO requests made prior to a fence will be visible to the remote PE before 
Put and AMO requests made following  the fence operation.\tabularnewline
\hline
Quiet & A generic term for OpenSHMEM routines a PE can invoke to ensure completion 
of remote memory access operations to symmetric data objects.\tabularnewline
\hline
Barrier & A generic term for OpenSHMEM routines in which all or some PEs collectively 
synchronize and ensure completion of all remote memory access operations prior to any 
PE returning from the call.\tabularnewline
\hline
Broadcast & A generic term for OpenSHMEM routines in which the root PE specifies a 
symmetric data object to be copied to a symmetric data object on one or more remote 
PEs.\tabularnewline
\hline
Collection & A generic term for OpenSHMEM routines in which all participating PEs 
get the result of concatenated symmetric objects contributed by each of the other 
PEs in another symmetric data object.\tabularnewline
\hline
Reduction & A generic term for OpenSHMEM routines in which all participating PEs 
get the result of an associative binary routine over elements of the specified symmetric 
data object on another symmetric data object.\tabularnewline
\hline
Lock & A generic term for OpenSHMEM routines which provide for setting, clearing, 
and testing a mutual exclusion lock.  \tabularnewline
\hline
Symmetric Heap & The memory region on which Symmetric Data Objects may be 
collectively allocated.\tabularnewline
\hline
Collective Routines & Communication or synchronization operations on an 
active set\tabularnewline
\hline
Active Set & The group of PEs that call collective routines. PEs that are not a part of 
the active set that call collective routines result in undefined behavior.\tabularnewline
\hline
Point-to-point synchronization routines & Mechanisms that provide synchronization 
between two PEs based on the value of a symmetric data object.\tabularnewline
\hline
