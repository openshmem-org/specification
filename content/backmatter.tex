...
symmetric data object which is to receive the data.  This capability aids programmability,
...
\FUNC{shmem\_init} has already been called, any subsequent calls to
...
The \FUNC{SHMEM\_CACHE} \ac{API}, listed below, was originally implemented for systems with cache management instructions.
...
The library constants, listed below, do not adhere to the \Cstd standard's reserved identifiers and the \Cpp
...
The environment variables, listed below, were deprecated in order to normalize the \openshmem \ac{API} to use
...
1.2 to Version 1.3. Many major changes to the specification were introduced in Version 1.3. These changes include non-blocking RMA operations, 
...
\FUNC{shmem\_finalize} library call. \FUNC{shmem\_finalize} provides a collective mechanism of
