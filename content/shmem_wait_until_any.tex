...
\apidescription{ 
    ...
    output, the element of \VAR{status} with the index given by the return
    value of \FUNC{shmem\_wait\_until\_any} is set to 1.  If \VAR{status} is a
    null pointer, it is ignored and all elements in \VAR{ivars} are included in
    the wait set.
}
