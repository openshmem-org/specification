...
\begin{C11synopsis}
size_t @\FuncDecl{shmem\_wait\_until\_any}@(TYPE *ivars, size_t nelems, int *status, int cmp,
    TYPE cmp_value);
\end{C11synopsis}
...
\begin{Csynopsis}
size_t @\FuncDecl{shmem\_\FuncParam{TYPENAME}\_wait\_until\_any}@(TYPE *ivars, size_t nelems, int *status, int cmp,
    TYPE cmp_value);
\end{Csynopsis}
...
\apidescription{ 
    ...
    included in the wait set, and will remain set to -1 on output.
    If all elements in \VAR{status} are set to 1 or
    \VAR{nelems} is 0, the wait set is empty and this routine returns
    \CONST{SIZE\_MAX}.  On output, the element of \VAR{status} with the index given by
    the return value of \FUNC{shmem\_wait\_until\_any} is set to 1, unless that
    element of \VAR{status} was set to -1 on input.  If
