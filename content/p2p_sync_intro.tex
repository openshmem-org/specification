...
to specify the type of \openshmem routine that is expected to update a variable
on the local \ac{PE} on which the point-to-point synchronization operation is
performed. The hints are named constants whose values are integer constant
expressions. The hint names and associated \openshmem routines that are expected
to update the variable on the local \ac{PE} are presented in Table~\ref{p2p-hints}.
\begin{table}[h]
  \begin{center}
    \begin{tabular}{ll}
      \hline
      Hint Name & Associated \openshmem routines \\ \hline
      \LibConstRef{SHMEM\_SIGNAL\_UPDATE}  & Section~\ref{subsec:shmem_put_signal}
      and Section~\ref{subsec:shmem_put_signal_nbi} \\ \hline
    \end{tabular}
