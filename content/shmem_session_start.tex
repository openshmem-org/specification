...
\apidescription{
    The \FUNC{shmem\_session\_start} routine begins a session on communication
    context \VAR{ctx} with hints requested via \VAR{options}.
    If a session is already started on a given context, another call to
    \FUNC{shmem\_session\_start} on that context combines any new options via a
    bitwise OR operation.
    Sessions on a communication context must be stopped with a call to
    \FUNC{shmem\_session\_stop} on the same context.
    Passing false or ambiguous \VAR{options} to a session should not result in
...
\apinotes{
    The \FUNC{shmem\_session\_start} routine is a hint for improving
