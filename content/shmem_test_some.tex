...
\begin{C11synopsis}
size_t @\FuncDecl{shmem\_test\_some}@(TYPE * restrict ivars, size_t nelems, size_t * restrict indices,
...
\begin{Csynopsis}
size_t @\FuncDecl{shmem\_\FuncParam{TYPENAME}\_test\_some}@(TYPE * restrict ivars, size_t nelems,
...
\apidescription{
    ...
    no conditions are satisfied.  This routine compares each element of the
    \VAR{ivars} array in the test set with the value \VAR{cmp\_value} according
    to the comparison operator \VAR{cmp} at the calling PE.  The order in which
    these elements are tested is unspecified.
