\apisummary{
  Test an array of variables on the local \ac{PE} to meet a specified test condition.
}
...
\apidescription{
    ...
    to by indices must be at least \VAR{nelems} long.
    ...
    array indicates which elements are excluded from the test set.  On output,
    the \VAR{status} array indicates which elements satisfied the test
    condition in \VAR{ivars} or were ignored.  If \VAR{status} is a null
    pointer, it is ignored and all elements in \VAR{ivars} are included in the
    test set.
}
