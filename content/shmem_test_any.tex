...
\begin{C11synopsis}
size_t @\FuncDecl{shmem\_test\_any}@(TYPE *ivars, size_t nelems, int *status, int cmp, TYPE cmp_value);
\end{C11synopsis}
where \TYPE{} is one of the point-to-point synchronization types specified by
Table \ref{p2psynctypes}.

\begin{Csynopsis}
size_t @\FuncDecl{shmem\_\FuncParam{TYPENAME}\_test\_any}@(TYPE *ivars, size_t nelems, int *status, int cmp,
    TYPE cmp_value);
\end{Csynopsis}
...
\apidescription{
    ...
    at the calling PE.  The order in which these elements are tested is
    ...
    empty and this routine returns \CONST{SIZE\_MAX}.  On output, the \VAR{status}
    array indicates which elements satisfied the test condition in \VAR{ivars}
    or were ignored.  If \VAR{status} is a null pointer, it is ignored and all
