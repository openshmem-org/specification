An \ac{AMO} \index{Atomic Memory Operation} is a one-sided communication mechanism that combines memory update
...
\begin{itemize}
\item
The \textit{fetching} routines \index{Fetching Routine} return the original value of, 
...
\item
The \textit{non-fetching} \index{Non-Fetching Routine} atomic routines update the remote 
memory in a single atomic operation.  A call to a non-fetching atomic routine issues the atomic
