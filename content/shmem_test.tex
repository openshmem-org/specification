...
\apidescription{
  ...
  Implementations must ensure that \FUNC{shmem\_test} does not return true before
  the update of the memory indicated by \VAR{ivar} is fully complete.
  Partial updates to the memory must not cause \FUNC{shmem\_test} to return true.
}
