...
\apidescription{
    ...
    other \openshmem routine may be called. Upon termination of initialization,
     the call to \FUNC{shmem\_init} must be matched with a
    call to \FUNC{shmem\_finalize}. After the first call to \FUNC{shmem\_init}, any
    subsequent calls to \FUNC{shmem\_init} in the same program results in undefined
    behavior.
}
...
\apinotes{
    ...
    \FUNC{shmem\_init} within a program results in undefined behavior, while 
    any subsequent calls to \FUNC{start\_pes} after the
    first one result in a no-op.
}
