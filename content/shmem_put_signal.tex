...
\apidescription{
    ...
    blocking or non-blocking variant of the put-with-signal routine that updates
    the \VAR{sig\_addr} signal data object using the same \VAR{sig\_op} signal
    update operator and any point-to-point synchronization routine that accesses
    the \VAR{sig\_addr} signal data object. With the above described atomicity
    guarantees, the following options can be used as the \VAR{sig\_op} signal
    operator.
