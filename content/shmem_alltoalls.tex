\apisummary{
    shmem\_alltoall is a collective routine where each \ac{PE} exchanges a fixed amount of strided data with all other
...
INTEGER nelems 
CALL SHMEM_ALLTOALL32(dest, source, dst, sst, nelems, PE_start, logPE_stride, PE_size, pSync)
CALL SHMEM_ALLTOALL64(dest, source, dst, sst, nelems, PE_start, logPE_stride, PE_size, pSync)
\end{Fsynopsis}
...
\apidescription{
    The \FUNC{shmem\_alltoalls} routines are collective routines; each \ac{PE} in
    ...
    \ac{PE} \VAR{j} is placed in the \VAR{dst}*\VAR{i}th block of the \VAR{dest} buffer on
    \ac{PE} \VAR{j}.
    ...
    Upon return from a \FUNC{shmem\_alltoalls} routine, the following is true
    for the local PE: The \VAR{dest} symmetric data object is updated.
} 
