...
interfaces. The main thread is the thread that invokes either \FUNC{shmem\_init} or
...
\begin{itemize}
\item
In \CONST{SHMEM\_THREAD\_MULTIPLE} and \CONST{SHMEM\_THREAD\_SERIALIZED} thread
levels, the \FUNC{shmem\_init} and \FUNC{shmem\_finalize} are invoked by the same thread. In
\CONST{SHMEM\_THREAD\_FUNNELED} thread level, the \FUNC{shmem\_init} and
\FUNC{shmem\_finalize} are invoked by the main thread. 
...
\item In \CONST{SHMEM\_THREAD\_SERIALIZED} and \CONST{SHMEM\_THREAD\_MULTIPLE} thread level, 
...
%, which are defined to call \FUNC{shmem\_barrier\_all}(\ref{sec:mem_routines}) must be treated as collective operations.
