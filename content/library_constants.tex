...
&
\color{ForestGreen}
An integer constant expression corresponding to a hint, specifying an update to
be expected as a signal on a variable on the local \ac{PE} through \openshmem
routines described in Section~\ref{subsec:shmem_put_signal} and
Section~\ref{subsec:shmem_put_signal_nbi}. See Section~\ref{subsec:p2p_intro}
