...
\apidescription{
    ...
    Table~\ref{mem-order} lists the operations for which the
    \FUNC{shmem\_quiet} routine ensures completion..
    All operations on symmetric data objects are guaranteed to be complete and
    visible to all \acp{PE} when \FUNC{shmem\_quiet} returns.
...
\apinotes{
    ...
    routines wait for the completion of outstanding writes to
    ...
    threads in a \ac{PE} that target symmetric memory
