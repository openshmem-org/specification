\apisummary{
    Waits for completion of all outstanding operations as listed in
    Table~\ref{quietops} to symmetric data objects issued by a \ac{PE}.
}
...
\apidescription{
    ...
    (as listed in Table~\ref{quietops}) on
    symmetric data objects issued by the calling \ac{PE} on the given context.
    All operations as specified in Table~\ref{quietops}
    to symmetric data objects are guaranteed to be completed and visible to all
  ...
  \begin{center}
    \begin{tabular}{l}
      \hline
      Supported Operations               \\ \hline
      Memory Store                       \\
      Blocking \OPR{Put}                 \\
      Blocking \OPR{AMO}                 \\
      Blocking \OPR{put-with-signal}     \\
      Non-blocking \OPR{Put}             \\
      Non-blocking \OPR{Get}             \\
      Non-blocking \OPR{AMO}             \\
      Non-blocking \OPR{put-with-signal} \\ \hline
    \end{tabular}
    \TableCaptionRef{List Of Operations Ensured Completion by \FUNC{shmem\_quiet}}
    \label{quietops}
  \end{center}
\end{table}
}
...
\apinotes{
    ...
    several operations as listed in Table~\ref{quietops}
    to symmetric data objects initiated by the calling
    ...
    routines wait for the completion of outstanding writes (from all operations
    as listed in Table~\ref{quietops}) to
    ...
    in a \ac{PE} that target symmetric memory
    (e.g. \PUT{}, \ac{AMO}, \OPR{put-with-signal}, memory stores,
    and nonblocking routines)
    and calls by threads in that \ac{PE} to
