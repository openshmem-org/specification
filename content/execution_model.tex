...
and remotely issued \openshmemem operations, result in undefined behavior.
Similarly, reads from a buffer that is in-use are allowed only when the buffer
was provided as a \CTYPE{const} argument to the \openshmem routine that it is
in-use by.  Otherwise, the behavior is undefined. Exceptions are made for
buffers that are in-use by \acp{AMOs}, as described in
Section~\ref{subsec:amo_guarantees}}.
