\apisummary{
    ...
    until all other \ac{PE}s arrive at the barrier.
}
...
\apidescription{   
    The \FUNC{shmem\_sync\_all} routine registers the arrival of a \ac{PE} at
    a barrier. Barriers are a fast mechanism for synchronizing all \ac{PE}s at
    once.  This routine causes a \ac{PE} to block until all \ac{PE}s
    have called \FUNC{shmem\_sync\_all}.
