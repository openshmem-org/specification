...
\apidescription{
    ...
    the test condition.  This routine \newtext{respectively} compares each of
    the \VAR{nelems} elements in the \VAR{ivars} array with \oldtext{the}
    \newtext{each} value \newtext{in} \VAR{cmp\_value\newtext{s}} according to
    the comparison operator \VAR{cmp} at the calling \ac{PE}.  The order in
    which these elements are tested is unspecified.  If an entry $i$ in
    \VAR{ivars} within the test set satisfies the test condition, a series of
    calls to \FUNC{shmem\_test\_any\newtext{\_vector}} must eventually return $i$.
