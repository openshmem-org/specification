...
feature of \openshmem is that data transfer operations are intrinsically
\textit{\textbf{one-sided}}, meaning that a local \ac{PE} executing
a data transfer routine does not require the participation of the remote \ac{PE}
to complete the routine. This feature allows for overlap between communication and
...
\begin{enumerate}
  ...
  \item \OPR{Query}: The local \ac{PE} requests the number of \acp{PE} running
      the same program and its unique integer identifier. 
  \item \OPR{Accessibility}: The local \ac{PE} may request if a remote \ac{PE} is
...
\begin{enumerate}
    \item \PUT: The local \ac{PE} specifies the \source{} data object (private
        or symmetric) copied to the symmetric data object on the remote
        \ac{PE}. 
  \item \GET: The local \ac{PE} specifies the symmetric data object on the remote
      \ac{PE} copied to a data object (private or symmetric) on the local
      \ac{PE}. 
\end{enumerate}
