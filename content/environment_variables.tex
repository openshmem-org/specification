...
\EnvVarDecl{SHMEM\_SYMMETRIC\_SIZE}
    ...
    ceiling of the product of the numeric prefix and the scaling factor per
    \ac{PE}. The allowed character suffixes for the scaling factor are as
    follows:
      \begin{itemize}
      ...
      recognized, so \enquote{20kk} will not produce the same result as
      \enquote{20m}. Usage of string \enquote{.5m} will yield the same result
      as the string \enquote{0.5m}.
