...
\apidescription{
    ...
    after posting the operation. If the request
    parameter is initialized to SHMEM\_REQUEST\_UNINITIALIZED, a new
    request will be created. There are no guarantees to the order of operations if multiple successive operations are posted.
    The operation is considered complete after a subsequent call to \FUNC{shmem\_wait\_nbe} for a given
    request handle returns, or after a call to \FUNC{shmem\_test\_nbe}
    returns with a non-zero return flag for the given request. After the
    completion of \FUNC{shmem\_wait\_nbe} or \FUNC{shmem\_test\_nbe} (with
    a non-zero flag), the data has been delivered to the \dest{} array
    on the local \ac{PE}.
}
