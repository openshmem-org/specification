...
\apidescription{
    ...
    \const{NULL} pointer, no action occurs.
    ...
    uninitialized.  When extending a block allocated with \FUNC{shmem\_calloc},
    \FUNC{shmem\_realloc} does not guarantee that the newly allocated portion
    of the block is initialized to all zero bits.
    If \VAR{ptr} is a \CONST{NULL} pointer, the
...
\apireturnvalues{
    ...
    (which may have moved); otherwise, it returns a \const{NULL} pointer.
