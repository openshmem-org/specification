...
objects. Table~\ref{mem-order} lists the operations for which \openshmem
memory ordering routines provide guarantees of ordering or completion.
...
\begin{table}[h]
  \begin{center}
    \begin{tabular}{|l|c|c|}
      ...
      Non-blocking \OPR{Put}             & X & X \\
      Non-blocking \OPR{AMO}             & X & X \\
      Non-blocking \OPR{put-with-signal} & X & X \\
      Non-blocking \OPR{Get}             &   & X \\ \hline
    \end{tabular}
    \TableCaptionRef{List of operations ensured ordering and/or delivery by
    Memory Ordering routines}
    \label{mem-order}
  \end{center}
\end{table}

Ordering and/or delivery of memory store operations are ensured only on contexts
created with certain options. For details, refer the description of context
options in Section~\ref{subsec:shmem_ctx_create}.
