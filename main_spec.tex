...
%commenting to test redmine's ability to track repo changes referencing issues, issue#8 for test purposes, seems to only work like #8
...
\cbstart \subsection{Library Setup, Exit, and Query Operations} \cbend %SP: Merging two operations
...
\cbstart \input{TEX_FILES/shmem_finalize.tex} \cbend %Swaroop
\label{subsec:shmem_finalize}
\cbstart \input{TEX_FILES/shmem_global_exit.tex} \cbend %Swaroop 
...
\cbstart \input{TEX_FILES/shmem_version.tex} %Manju
\label{subsec:shmem_version}
\input{TEX_FILES/shmem_name.tex} \cbend %Manju
...
\cbstart \chapter{\openshmem Specification and Deprecated API }
\input{INTRODUCTION/deprecated_API.tex} \cbend
\label{sec:dep_api}
\cbstart \chapter{Changes to this Document}
\input{INTRODUCTION/changelog1.2} \cbend
%\clearpage
%\startchap
