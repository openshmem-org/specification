...
The number of \ac{PE}s in the \openshmem{} program is specified at runtime by the user. \ac{PE}s can be implemented as an OS process or OS thread All \ac{PE}~\footnote{As long as the memory model and execution model 
of \openshmem is followed.} and may be mapped 1-to-1 to hardware cores/threads or less. %As long as the memory model and execution model of \openshmem is followed.
