...
manner from zero to the total number of \ac{PE}s minus 1. \ac{PE} identifiers are used for some \openshmem{} library calls (such as collective synchronization calls) or to dictate a control flow for \ac{PE}s using constructs of \Clang{} or \Fortran. The identifiers are fixed for the lifecycle of the \openshmem{} program.
