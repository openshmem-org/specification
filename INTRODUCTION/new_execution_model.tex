...
where each \ac{PE} can take a different execution path. A \ac{PE} can be implemented using an OS process or an OS thread. 
However, implementing a \ac{PE} using an OS thread requires compiler techniques to implement the \openshmem{} memory model which is 
not portable, this approach is strongly discouraged.
