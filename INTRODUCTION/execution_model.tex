...
%in parallel. 
An OpenSHMEM program consists of a set of processes, called \ac{PEs} where each process has its own 'local' (private) memory and symmetric memory regions that may be accessible by any PEs.
Although all PEs executes in an SPMD-like fashion, they may have different execution paths and will execute asynchronously following \Fortran{} or program execution in \Clang{}. Each PE may be mapped to many to one hardware cores/threads or less. In \openshmem the number of PEs is specified at runtime.

\openshmem requires initialization before using any of the OpenSHMEM library
routines by calling \textbf{start\_pes()} during the initialization phase of a program. 
The PEs do not exist till after \textbf{start\_pes()} returns. \textbf{start\_pes()} performs any required initialization steps, such as setting up the symmetric heap for every PE and creating
the PE numbers which act like unique global identifiers for the duration of the program. These PE identifiers are integers assigned in a monotonically
increasing manner from zero to the total number of PEs minus 1. PE identifiers may be used for library calls synchronizations, or control flow constructs of \Clang{} or \Fortran{}
