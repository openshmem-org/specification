...
%in parallel. 
An OpenSHMEM program consists of a set of processes, called \ac{PE}s where each process has its own 'local' (private) memory and symmetric memory regions that may be accessible by any PEs.
Although \openshmem follows the SPMD execution model, different \ac{PE}s may have different execution paths and will execute asynchronously following \Fortran{} or program execution in \Clang{}. Each PE may be mapped to many to one hardware cores/threads or less. In \openshmem the number of \ac{PE}s is specified at runtime.
...
The PEs do not exist till after \textbf{start\_pes()} returns. \textbf{start\_pes()} performs any required initialization steps, such as setting up the symmetric heap for every PE and creating and assigning PE numbers which act like unique global identifiers for the duration of the program. These PE identifiers are integers assigned in a monotonically increasing manner from zero to the total number of PEs minus 1. PE identifiers are used for other \openshmem library calls (such as collective synchronization) or to dictate a definite control flow for \ac{PE}s using constructs of \Clang{} or \Fortran{}. Some collective operations require the creation of an active set, which is  group of PEs that is involved in the execution of a collective operation. These collective routines assume that only \ac{PE}s in the active set call the routine. If a \ac{PE} not in the active set calls a SHMEM collective routine,  undefined  behavior results.
...
issuing any explicit \openshmem calls. \openshmem communication calls involving that PE must progress
...
active set.  You can use  \FUNC{shmem\_barrier}  or  \FUNC{shmem\_barrier\_all}  to
...
\item  If the same collective function is called multiple times with the
          same active set, the calls may alternate between two \VAR{pSync} arrays.
