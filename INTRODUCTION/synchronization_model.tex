...
In the presence of the \openshmem's one-sided communication, synchronization and ordering become critical. The updates via \FUNC{shmem\_puts} cannot be guaranteed until some form of synchronization or ordering is introduced by the application programmer. The table below gives the different synchronization and ordering options, and situations where they may be useful.\\
...
{}
&
All Puts issued to same \ac{PE}  are guaranteed to be completed before Puts (to the same \ac{PE}) issued after the \FUNC{fence} call. \tabularnewline
