...
\tabularnewline
\hline 
Collective synchronization over an active set & shmem\_barrier & {All local and remote memory operations issued by all \ac{PE}s within the active set are guaranteed to be completed before any \ac{PE} in the active set returns from the call. Additionally no \ac{PE} my return from the barrier till all \ac{PE}s in the \activeset have called the same barrier call. This operation should be used when synchronization as well as completion of local stores and remote memory updates via OpenSHMEM is required over a sub-set of the executing \ac{PE}s.} \tabularnewline
\hline 
