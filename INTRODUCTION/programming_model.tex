...
\begin{enumerate}
\item \textbf{Library Setup and Query Routines}

\begin{enumerate}
\item \FUNC{Initialization}: The \openshmem library environment it initialized. 
\item \FUNC{Query}: The local \ac{PE} may get number of \acp{PE} running the same application and its unique integer identifier. 
\item \FUNC{Accessibility}: The local \ac{PE} can find out if a PE is executing the same binary, or if a particular symmetric data object can be accessed by a remote \ac{PE}, or may obtain a pointer to a symmetric data object on the specified remote PE on shared memory systems.
\end{enumerate}
...
\item \textbf{Synchronization and Ordering Mechanisms }
\begin{enumerate}
