...
restored to its original contents.  The reuse of \VAR{pSync} array is allowed
for a \ac{PE}, if all previous collective routines using the \VAR{pSync} array
...
previous collective routines. The \FUNC{shmem\_barrier} routine allows the same
\VAR{pSync} array to be used on consecutive calls as long as the \ac{PE}
